...
const Selector = () => {
  ...
  return (
    <div className='w-72 font-medium h-80'>
        ...
        className={`bg-white w-full p-2 flex items-center justify-between rounded ${!selected && 'text-gray-500'}`}>
        ...
        <ul className={`bg-white mt-2 overflow-y-auto ${open ? 'max-h-60' : 'max-h-0'}`}>
          ...
          <AiOutlineSearch size={18} className="text-gray-700"/>
            <input value={inputValue} onChange={(e)=>setInputValue(e.target.value.toLowerCase())} type="text" placeholder='Enter country name' className='placeholder:text-gray-500 p-2 outline-none'/>
          </div>
